...
void getOrientation2(bool);
void getTilt(double[4]);
void quartToEuler(double[4]);
...
void loop() {
  ...
  Serial.print("Position robot at ");
  Serial.print(testAngles[index]);
  Serial.println(" degrees and hold...");

  delay(settleTime);  // Give you time to position it

  Serial.println("Recording...");

  for (int i = 0; i < numSamples; i++) {
    getOrientation2(true);  // Your existing function
    delay(10);  // Control sample rate
  }
...
void getOrientation2(bool calibrated) {
...
void getTilt(double qIn[4]) {
  ...
  double tiltAngle = acosf(up_z) * RAD_TO_DEG;
  double tiltHeading = atan2f(up_y, up_x) * RAD_TO_DEG;
  Serial.print(tiltAngle); //Serial.print("   "); Serial.println(tiltHeading);
  Serial.println("");
}

// void quartToEuler(double qIn[4]) {
//   double q[4];
//   double norm = sqrt(qIn[0]*qIn[0] + qIn[1]*qIn[1] + qIn[2]*qIn[2] + qIn[3]*qIn[3]);
//   for (int i=0;i<4;i++){
//     q[i] = qIn[i] / norm;
...
//   orient.roll = atan2(
//     2.0 * (q[0] * q[1] + q[2] * q[3]),
//     1.0 - 2.0 * (q[1] * q[1] + q[2] * q[2])) * RAD_TO_DEG;
// }
